...
from plora.manifest import Manifest, LoraConfig as ManLoraCfg, ArtifactsInfo, TrainMeta, MetricsInfo, SafetyInfo, SignerInfo, CompatibilityInfo
...
def train(
    ...
    *,
    ...
    pairs = get_dataset(domain, max_samples=samples if samples and samples > 0 else None)
    ...
    l_cfg = LoraConfig(r=rank, lora_alpha=rank * 2, target_modules=target_modules, lora_dropout=0.1)
    ...
    # Metrics
    ppl_before = perplexity(AutoModelForCausalLM.from_pretrained(base_model, torch_dtype=dtype, device_map={"": device}), tok, dev_pairs)
    ...
    # Manifest
    sha_hex = torch.sha256(adapter_path.read_bytes()).hexdigest() if hasattr(torch, 'sha256') else __import__('hashlib').sha256(adapter_path.read_bytes()).hexdigest()
    manifest = Manifest(
        schema_version=0,
        ...
        peft_format="lora",
        lora=ManLoraCfg(r=l_cfg.r, alpha=l_cfg.lora_alpha, dropout=l_cfg.lora_dropout, target_modules=l_cfg.target_modules),
        artifacts=ArtifactsInfo(filename=adapter_path.name, sha256=sha_hex, size_bytes=adapter_path.stat().st_size),
        train_meta=TrainMeta(seed=SEED, epochs=epochs, dataset_id=domain, sample_count=len(pairs), timestamp_unix=int(time.time())),
        metrics=MetricsInfo(val_ppl_before=ppl_before, val_ppl_after=ppl_after, delta_ppl=delta_ppl, val_em=None, val_chrf=None),
...
def main():
    ...
    parser.add_argument("--samples", type=int, default=None, help="Number of samples to load (omit for full dataset)")
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--sign", action="store_true")
    parser.add_argument("--shuffle-labels", action="store_true", help="Permute answers within the batch for placebo B training.")
    ...
    if args.sign:
        ...
        manifest.signer = manifest.signer.copy(update={"algo": "RSA-PSS-SHA256", "signature_b64": sig})
