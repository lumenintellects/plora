...
def main():
    ...
    if not tmods:
        raise ValueError("No compatible LoRA target modules found for base model; check architecture and scheme")
    l_cfg = LoraConfig(r=ns.rank, lora_alpha=ns.rank * 2, target_modules=tmods, lora_dropout=0.1)
    ...
    manifest = Manifest(
        ...
        train_meta=TrainMeta(seed=42, epochs=ns.epochs, dataset_id="+".join(ns.domains), sample_count=len(pairs), timestamp_unix=int(time.time())),
