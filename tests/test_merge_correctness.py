...
def _make_adapter(base: str, out_dir: Path):
    ...
    model = AutoModelForCausalLM.from_pretrained(base, torch_dtype=dtype, device_map={"": device})
...
def test_merge(tmp_path: Path):
    ...
    out = merged.generate(max_length=2, do_sample=False, num_beams=1, input_ids=input_ids)
